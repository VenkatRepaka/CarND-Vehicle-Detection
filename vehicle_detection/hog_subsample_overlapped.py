...
for scale in scales:
    find_cars(img, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size,
                        hist_bins)
for corners in all_boxes:
    cv2.rectangle(draw_img, corners[0],
                  corners[1], (0, 0, 255), 6)
pickle.dump(all_boxes, open("bbox_pickle.p", "wb"))
plt.imshow(draw_img)
plt.show()