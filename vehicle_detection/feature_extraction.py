...
def find_cars_boxes(img, ystart, ystop, scale, svc, X_scaler, color_space, orient, pix_per_cell, cell_per_block,
...
def add_heatmap_and_threshold(draw_img, bbox_list, threshold):
    ...
    heat_map_queue.append(heatmap)
    heatmap = np.sum(np.array(heat_map_queue), axis=0)
